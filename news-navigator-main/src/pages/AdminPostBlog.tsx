...
const AdminPostBlog = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post<BlogResponse>("/admin/blogs", payload);
      toast({ title: "Blog posted", description: `Created: ${res.title}` });
      navigate("/admin/dashboard");
    } catch (err: any) {
      toast({ title: "Post failed", description: err.message || "Unable to post blog" });
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div className="space-y-2">
                ...
                <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Blog title" />
              </div>
              <div className="space-y-2">
              ...
              <Button type="submit" disabled={submitting} className="w-full">{submitting ? "Posting..." : "Post Blog"}</Button>
            </form>
          </CardContent>
        </Card>
      </div>
